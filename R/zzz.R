.onAttach <- function(libname, pkgname) {
  ...
  if(!is.null(dsn_values)) {suppressWarnings(set_dsn(dsn = dsn_values))}
  dsn_check <- is.null(options()$ez.dsn)
  if (dsn_check) {
    packageStartupMessage(crayon::red("\U2717 - Detecting no DSNs!"))
    packageStartupMessage(crayon::red(strwrap("Make sure to set your DSN
                                              using set_dsn() or save_dsn()")))
  } else {
